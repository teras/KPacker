...
object MacX64Configurator : Configurator {
    ...
    private suspend fun handleIcon(resourcesDir: okio.Path, app: Application) {
        ...
        if (iconPath != null && fs.exists(iconPath)) {
            ...
            val iconconvertCommand = containerRunner.run(
                "run", "--rm", "-t",
                "-v", "${iconPath.parent}:/input",
                "-v", "${targetIconPath.parent}:/output",
                "docker.io/teras/appimage-builder",
                "sh", "-c", "iconconvert /input/${iconPath.name} /output/${targetIconPath.name} && chown \$(stat -c '%u:%g' /input) /output/${targetIconPath.name}"
            )
    ...
    private suspend fun createDMGFromTemplate(outputDir: String, app: Application, appBundlePath: okio.Path, dmgPath: okio.Path, templatePath: String) {
        val fs = FileSystem.SYSTEM
        ...
        try {
            ...
            val mkfsCommand = Command(
                "docker", "run", "--rm", "-t",
                "-v", "${uncompressedDmgPath.parent}:/work",
                "docker.io/teras/appimage-builder",
                "sh", "-c", "mkfs.hfsplus -v ${app.name} /work/${uncompressedDmgPath.name} && chown \$(stat -c '%u:%g' /work) /work/${uncompressedDmgPath.name}"
            )
            ...
            try {
                ...
                val existingApps = fs.list(mountPointPath).filter { it.name.endsWith(".app") && fs.metadata(it).isDirectory }
            ...
            val containerCommand = Command(
                ...
                "-v", "$outputDir:/data",
                "docker.io/teras/appimage-builder",
                "sh", "-c", "dmg /data/${app.name}-${app.version}-uncompressed.dmg /data/${app.name}-${app.version}.dmg && chown \$(stat -c '%u:%g' /data) /data/${app.name}-${app.version}.dmg"
            )
    ...
    private suspend fun createDMGFromScratch(outputDir: String, app: Application, appBundlePath: okio.Path, dmgPath: okio.Path) {
        val fs = FileSystem.SYSTEM
        val uncompressedDmgPath = dmgPath.parent!! / "${app.name}-${app.version}-uncompressed.dmg"
    ...
    private suspend fun signSingleItem(path: okio.Path, app: Application, withRuntime: Boolean) {
        ...
        val command = Command(
            ...
            "docker.io/teras/appimage-builder",
            "sh", "-c", "${rcodesignArgs.joinToString(" ")} && chown \$(stat -c '%u:%g' /work) /work/${path.name}"
        )
    ...
    private suspend fun notarizeMacOSApp(path: okio.Path, app: Application) {
        ...
        val notarizeCommand = Command(
            "docker", "run", "--rm", "-t",
            "-v", "${path.parent}:/work",
            "-v", "${notaryJson.toPath().parent}:/config",
            "docker.io/teras/appimage-builder",
            "sh", "-c", "rcodesign notary-submit --api-key-file /config/${notaryJson.toPath().name} --staple /work/${path.name} && chown \$(stat -c '%u:%g' /work) /work/${path.name}"
        )
